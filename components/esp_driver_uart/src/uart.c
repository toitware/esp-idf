    ...
    while (1) {
        ...
        if (uart_intr_status & UART_INTR_TXFIFO_EMPTY) {
            ...
            }
            {
                ...
                size_t max_size = xRingbufferGetMaxItemSize(p_uart->tx_ring_buf);
                size_t current_free = xRingbufferGetCurFreeSize(p_uart->tx_ring_buf);
                if (p_uart->tx_len_tot == 0 && max_size == current_free) {
                    uart_event.type = UART_TX_DONE;
                }
            }
            uart_event.type = UART_TX_DONE;  // Toit extension.
        }
#if SOC_UART_SUPPORT_WAKEUP_INT
